...
func createTestSchedulingResult(metadata *fwkdl.EndpointMetadata) *schedulingtypes.SchedulingResult {
	...
	mockPod := createTestEndpoint(metadata.Key.NamespacedName.Name, kvUsage, runningRequests, waitingQueue)
...
func TestPredictedLatency_PreRequest_Success(t *testing.T) {
	...
	router.runningRequestLists.Store(endpoint.GetMetadata().Key, newRequestPriorityQueue())
...
func TestPredictedLatency_PreRequest_AddsToQueue(t *testing.T) {
	...
	value, exists := router.runningRequestLists.Load(endpoint.GetMetadata().GetNamespacedName())
...
func TestPredictedLatency_StreamingMode_ResponseBody_FirstToken(t *testing.T) {
	...
	router.runningRequestLists.Store(endpoint.GetMetadata().Key, queue)
...
func TestPredictedLatency_StreamingMode_ResponseBody_FinalToken_Success(t *testing.T) {
	...
	queue := newRequestPriorityQueue()
	router.runningRequestLists.Store(endpoint.GetMetadata().GetNamespacedName(), queue)
	queue.Add(request.Headers[reqcommon.RequestIdHeaderKey], 50.0)
...
func TestPredictedLatency_MultipleRequests_DifferentPods(t *testing.T) {
	...
	value1, exists1 := router.runningRequestLists.Load(endpoint1.GetMetadata().GetNamespacedName())
	value2, exists2 := router.runningRequestLists.Load(endpoint2.GetMetadata().GetNamespacedName())
