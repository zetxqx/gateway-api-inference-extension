...
func TestPickWeightedRandomPicker(t *testing.T) {
	...
	endpoint4 := fwksched.NewEndpoint(&fwkdl.EndpointMetadata{Key: fwkplugin.NewEndpointKey("pod4", "", 0)}, nil, nil)
	endpoint5 := fwksched.NewEndpoint(&fwkdl.EndpointMetadata{Key: fwkplugin.NewEndpointKey("pod5", "", 0)}, nil, nil)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, endpoint := range test.input {
				podName := endpoint.GetMetadata().GetKey().String()
				if totalScore > 0 {
			...
			for _, endpoint := range test.input {
				endpointName := endpoint.GetMetadata().GetKey().String()
			...
			for range testIterations {
				...
				selectedEndpointName := result.TargetEndpoints[0].GetMetadata().GetKey().String()
