...
func (e *SimpleTokenEstimator) Estimate(request *framework.InferenceRequest) int64 {
	...
	case request.Body != nil:
		...
		var chars int
		switch {
		case request.Body.LLMRequestBody != nil && request.Body.LLMRequestBody.Completions != nil:
			chars = len(request.Body.LLMRequestBody.Completions.Prompt)
		case request.Body.LLMRequestBody != nil && request.Body.LLMRequestBody.ChatCompletions != nil:
			for _, m := range request.Body.LLMRequestBody.ChatCompletions.Messages {
