...
func TestInferencePoolReconciler(t *testing.T) {
	...
	for _, epf := range factories {
		...
		if diff := diffStore(ds, diffStoreParams{wantPool: endpointPool1, wantEndpoints: []string{"pool1-ns/pod1:8080", "pool1-ns/pod2:8080"}}); diff != "" {
		...
		if diff := diffStore(ds, diffStoreParams{wantPool: newEndpointPool1, wantEndpoints: []string{"pool1-ns/pod5:8080"}}); diff != "" {
			t.Errorf("Unexpected diff (+got/-want): %s", diff)
		}
		...
		newEndpointPool1 = pool.InferencePoolToEndpointPool(newPool1)
		if diff := diffStore(ds, diffStoreParams{wantPool: newEndpointPool1, wantEndpoints: []string{"pool1-ns/pod5:9090"}}); diff != "" {
			t.Errorf("Unexpected diff (+got/-want): %s", diff)
		}
...
func diffStore(store datastore.Datastore, params diffStoreParams) string {
	...
	for _, em := range store.PodList(datastore.AllPodsPredicate) {
		gotEndpoints = append(gotEndpoints, em.GetMetadata().GetKey().String())
	}
