...
func TestIndexer_AddAndGet(t *testing.T) {
	...
	server := Server{
		...
		numOfBlocks: 3, // Set max cache size.
	}
	...
	serverIDs := i.Get(hash1)
	assert.Contains(t, serverIDs, server.ServerID, "Cache should contain the added server")
	...
	assert.Equal(t, 3, i.podToLRU[server.ServerID].Len(), "Cache size should be 3 after adding an entry")

	// Add another entry to the cache, which should evict the first one due to max size.
	i.Add([]BlockHash{BlockHash(4)}, server)
	assert.Equal(t, 3, i.podToLRU[server.ServerID].Len(), "Cache size should still be 3 after adding an entry")

	serverIDs = i.Get(BlockHash(5))
	assert.Empty(t, serverIDs, "Cache should not contain non-existent hash")
}
...
func TestIndexer_RemovePodAndEviction(t *testing.T) {
	...
	server2 := Server{
		ServerID: ServerID{Namespace: "default", Name: "server2"},
	}
